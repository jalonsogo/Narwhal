...
export function ToolDetailDialog({ isOpen, onClose, toolName }: ToolDetailDialogProps) {
  ...
  console.log('ToolDetailDialog - toolName:', toolName);
  console.log('ToolDetailDialog - category found:', category);
  console.log('ToolDetailDialog - isOpen:', isOpen);

  if (!category) {
    console.warn('No category found for tool:', toolName);
    return (
      <Dialog.Root open={isOpen} onOpenChange={onClose}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-xl w-[600px] max-h-[80vh] overflow-hidden z-50">
            <div className="p-6">
              <p className="text-red-600">Tool category "{toolName}" not found</p>
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-green-100 rounded flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="text-green-700 font-mono text-sm font-bold">
                        {index + 1}
                      </span>
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      {Object.keys(func.arguments).length === 0 && (
                        <div className="text-xs text-gray-400 italic">No arguments</div>
                      )}
                    </div>
                  </div>
                </div>
